...
export const getAllCatController = async (req, res) => {
  try {
    ...
    res.status(500).send({ success: false, message: "Erorr in get All Categpry API", error });
  }
};
...
export const deleteCatController = async (req, res) => {
  try {
    ...
    res.status(200).send({ success: true, message: "category Deleted succssfully" });
  } catch (error) {
    res.status(500).send({ success: false, message: "error in Dlete Cat APi", error });
  }
};
