...
export const createResturantController = async (req, res) => {
  try {
    ...
    res.status(201).send({ success: true, message: "New Resturant Created successfully" });
  } catch (error) {
...
export const getAllResturantController = async (req, res) => {
  try {
    ...
    if (!resturants) {
      return res.status(404).send({ success: false, message: "No Resturant Availible" });
    }
    res.status(200).send({ success: true, totalCount: resturants.length, resturants });
  } catch (error) {
    res.status(500).send({ success: false, message: "Error In Get ALL Resturat API", error });
  }
};
...
export const getResturantByIdController = async (req, res) => {
  try {
    ...
    if (!resturantId) {
      return res.status(404).send({ success: false, message: "Please Provide Resturnat ID" });
    }
    ...
    res.status(500).send({ success: false, message: "Error In Get Resturarnt by id api", error });
  }
};
...
export const deleteResturantController = async (req, res) => {
  try {
    const resturantId = req.params.id;
    if (!resturantId) {
      return res.status(404).send({ success: false, message: "No Resturant Found OR Provide Resturant ID" });
    }
    await resturantModel.findByIdAndDelete(resturantId);
    res.status(200).send({ success: true, message: "Resturant Deleted Successfully" });
  } catch (error) {
    res.status(500).send({ success: false, message: "Eror in delete resturant api", error });
  }
};
